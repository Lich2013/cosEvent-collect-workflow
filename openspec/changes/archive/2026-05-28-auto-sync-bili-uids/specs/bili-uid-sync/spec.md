...
- **WHEN** 匹配新人 Coser 时，最佳匹配的 B站 粉丝仅有 30 个，但该候选人名字精确匹配且有官方认证
...
#### Scenario: 快速降级至 DOM 解析
- **WHEN** 检索因 B站 反爬触发 WAF 阻断，Ajax 接口在 2.5 秒内未响应
- **THEN** 系统秒级超时断开拦截器，并优雅滑入 DOM 节点解析以继续提取数据
