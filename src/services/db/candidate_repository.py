...
class CandidateRepository:
        ...
        try:
            ...
            if row:
                ...
                if status == "pending":
                    ...
                    cursor.execute(
                        """
                        SELECT platform, source_ref, matched_bili_uid, matched_weibo_uid, matched_xhs_uid, match_score 
                        FROM coser_candidates 
                        WHERE id = ?;
                        """,
                        (cand_id,)
                    )
                    exist_plat, exist_ref, exist_bili, exist_weibo, exist_xhs, exist_score = cursor.fetchone()
                    ...
                    cursor.execute(
                        ...
                        SET platform = ?, source_ref = ?, matched_bili_uid = ?, matched_weibo_uid = ?, matched_xhs_uid = ?, match_score = ?, created_at = ?
                        WHERE id = ?;
                        """,
                        (merged_plat, merged_ref, merged_bili, merged_weibo, merged_xhs, merged_score, now_str, cand_id)
                    )
                    conn.commit()
                    return True
                else:
            ...
            cursor.execute(
                ...
                (name, platform, source_ref, matched_bili_uid, matched_weibo_uid, matched_xhs_uid, match_score, status, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, 'pending', ?);
                """,
                (name, platform, source_ref, matched_bili_uid, matched_weibo_uid, matched_xhs_uid, match_score, now_str)
            )
            conn.commit()
            return True
    ...
    def list_candidates(status: str = "pending") -> list[dict]:
        ...
        try:
            cursor.execute(
                """
                SELECT id, name, platform, source_ref, matched_bili_uid, matched_weibo_uid, matched_xhs_uid, match_score, status, created_at
                FROM coser_candidates
                WHERE status = ?;
                """,
                (status,)
